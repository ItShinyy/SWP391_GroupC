    ...
    <body class="map-page">
        ...
        <div class="app-container">
            <aside class="sidebar">
                ...
                <div class="search-box">
                    <button type="button" onclick="locateUser()"><span class="refresh-icon"></span>Cập nhật vị
                        trí</button>
                </div>
        ...
        <script>
            ...
            const CONFIG = {
                ...
                specialityPattern: 'general|dermatology'
            };
            ...
            async function findNearbyClinics() {
                try {
                    showLoading('Đang tìm cơ sở y tế gần bạn...');
                    showUserOnMap();

                    const radius = CONFIG.radius;
                    const specialityFilter = CONFIG.specialityPattern
                        ? '["healthcare:speciality"~"^(' + CONFIG.specialityPattern + ')$"]'
                        : '';
                    ...
                    const query = '[out:json][timeout:25];'
                        + '('
                        + 'nwr(around:' + radius + ',' + userLat + ',' + userLng + ')["amenity"~"^(hospital|clinic|doctors)$"]'
                        + specialityFilter + ';'
                        + 'nwr(around:' + radius + ',' + userLat + ',' + userLng + ')["healthcare"~"^(hospital|clinic|doctor)$"]'
                        + specialityFilter + ';'
                        + ');'
                        + 'out center tags;';
                    //gửi request đến overpass, await để đợi API nó trả kết quả về

                    const response = await fetch(CONFIG.overpassUrl, {
                        method: 'POST',
                        // Khai báo kiểu dữ liệu gửi lên là form-urlencoded,
                        //giống kiểu dữ liệu form HTML gửi đi.
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'
                        },
                        //port cái query lên server
                    ...
                    if (displayedCount === 0) {
                        await showDatabaseClinics('Không tìm thấy cơ sở nào trên OpenStreetMap.');
                        return;
                    }
                    hideLoading();
                } catch (error) {
            ...
            async function showDatabaseClinics(reason) {
                try {
                    ...
                    // đọc dữ liệu trả về từ database API
                    const clinics = await response.json();
                    //bật fallback, xóa vị trí người dùng
                    fallbackMode = true;
                    ...
                    // duyệt từng clinics trong danh sách
            ...
            function displayClinics(elements) {
                ...
                let clinics = elements.map(function (element) {
                    ...
                    return {
                        ...
                        searchUrl: buildClinicSearchUrl(tags.name, buildAddress(tags)),
                ...
                document.getElementById('stats-text').innerHTML = fallbackMode
            ...
            // Lấy website từ OSM hoặc database; tự thêm https:// nếu dữ liệu chỉ có domain.
