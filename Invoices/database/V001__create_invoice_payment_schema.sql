...
/*
    Simple payment schema for the current SkinAI database.

    Design goals:
    - Keep the payment module easy to use: only 2 tables.
    - Follow the existing schema.sql style: lowercase table names and UNIQUEIDENTIFIER ids.
    - Link invoices directly to appointments(id).
    - Store VNPay payment attempts safely in payments.
    - Do not modify or delete old data.
*/

IF OBJECT_ID(N'dbo.payments', N'U') IS NOT NULL
BEGIN
    PRINT N'dbo.payments already exists. No changes were made.';
END;

IF OBJECT_ID(N'dbo.invoices', N'U') IS NOT NULL
BEGIN
    PRINT N'dbo.invoices already exists. No changes were made.';
END;
GO
...
BEGIN
    ...
    PRINT N'dbo.invoices was created.';
END;
GO
...
BEGIN
    ...
    PRINT N'dbo.payments was created.';
END;
GO
