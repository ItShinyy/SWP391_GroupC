...
public class DashboardApiController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            for (int i = 0; i < recentList.size(); i++) {
                ...
                json.append("{");
                json.append("\"id\":\"").append(FormatUtil.escapeJson(dr.getId())).append("\",");
                json.append("\"patientName\":\"").append(FormatUtil.escapeJson(dr.getPatientName())).append("\",");
                json.append("\"diseaseName\":\"").append(FormatUtil.escapeJson(dr.getDiseaseName())).append("\",");
                json.append("\"riskLevel\":\"").append(FormatUtil.escapeJson(dr.getRiskLevel())).append("\",");
    ...
    private String mapToJsonString(Map<String, Integer> map) {
        ...
        for (Map.Entry<String, Integer> entry : map.entrySet()) {
            if (!first) sb.append(",");
            sb.append("\"").append(FormatUtil.escapeJson(entry.getKey())).append("\":").append(entry.getValue());
            first = false;
        }
        sb.append("}");
        return sb.toString();
