...
/** Patient-only endpoint for submitting an issue to the administration team. */
@MultipartConfig(
...
public class IssueReportController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            auditLogDAO.createLog(user.getId(), "ISSUE_REPORT_CREATE", "issue_reports", reportId,
                    "Mã báo lỗi: " + report.getReportCode(), null,
    ...
    private String saveImage(HttpServletRequest request) throws IOException, ServletException {
        ...
        String imageUrl = FileStorageUtil.saveFile(imagePart, uploadDirectory);
        if (imageUrl == null) {
            request.setAttribute("uploadError", "Không thể lưu ảnh đính kèm. Vui lòng thử lại.");
        }
        return imageUrl;
    }
    ...
    private User requireReporter(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (!"PATIENT".equals(user.getRole()) && !"USER".equals(user.getRole())
