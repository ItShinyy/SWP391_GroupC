...
<div class="container-fluid py-4 px-4">
    ...
    <form method="post" action="${pageContext.request.contextPath}/doctor/schedule">
                        ...
                        <c:forEach var="day" items="${weekDays}" varStatus="dayLoop">
                            <div class="day-column flex-fill ${day.isToday ? 'day-today' : ''}">
                                ...
                                <div class="slot-item ${day.morningAvailable ? 'slot-available' : 'slot-unavailable'} mt-3">
                                    ...
                                    <c:if test="${day.morningCount > 0}">
                                        <div class="appointment-count mt-1" title="${day.morningCount} lịch hẹn">
                                            ${day.morningCount}
                                        </div>
                                    </c:if>
                                </div>
                                ...
                                <div class="slot-item ${day.afternoonAvailable ? 'slot-available' : 'slot-unavailable'}">
                                    ...
                                    <c:if test="${day.afternoonCount > 0}">
                                        <div class="appointment-count mt-1" title="${day.afternoonCount} lịch hẹn">
                                            ${day.afternoonCount}
                                        </div>
                                    </c:if>
                                </div>
                                ...
                                <div class="slot-item ${day.eveningAvailable ? 'slot-available' : 'slot-unavailable'}">
                                    ...
                                    <c:if test="${day.eveningCount > 0}">
                                        <div class="appointment-count mt-1" title="${day.eveningCount} lịch hẹn">
                                            ${day.eveningCount}
                                        </div>
                                    </c:if>
                                </div>
        ...
        <div class="d-flex flex-wrap gap-4 mt-3 px-1">
            ...
            <div class="d-flex align-items-center gap-2">
                <div class="appointment-count" style="width: 14px; height: 14px; font-size: 0.5rem;">3</div>
                <span style="font-size: 0.8rem; font-weight: 600; color: #64748b;">Số lịch hẹn</span>
            </div>
        </div>
    </form>
    ...
    <div class="card border-0 shadow-sm rounded-4 mt-4">
        <div class="card-header bg-white border-0 p-4 pb-3">
            <h5 class="fw-bold mb-0">
                <i class="fa-solid fa-calendar-day me-2 text-primary"></i>Lịch Hẹn Sắp Tới
            </h5>
        </div>
        <div class="card-body p-0">
            <c:choose>
                            ...
                            <tbody>
                                <c:forEach var="uApt" items="${upcomingAppointments}">
                                    <tr style="transition: background 0.15s ease;">
                                        ...
                                        <td style="padding: 1rem; vertical-align: middle; border-bottom: 1px solid #f1f5f9;">
                                            <span class="badge-status badge-accepted"><i class="fa-solid fa-check me-1"></i>Đã xác nhận</span>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </tbody>
