...
public class FeedbackController extends HttpServlet {
    ...
    private void handleShowCreateFeedback(HttpServletRequest req, HttpServletResponse resp, User user) 
        ...
        if (!ensureVisitCompleted(appointment)) {
        ...
        try {
            ...
            if (feedbackId != null) {
                ...
                auditLogDAO.createLog(user.getId(), "FEEDBACK_CREATE", "feedbacks", feedbackId,
                                    null, "Tạo đánh giá cho lịch hẹn: " + appointmentId, null, clientIp, userAgent);
        ...
        try {
            ...
            if (updated) {
                ...
                auditLogDAO.createLog(user.getId(), "FEEDBACK_UPDATE", "feedbacks", feedbackId,
                                    null, "Cập nhật đánh giá", null, clientIp, userAgent);
    ...
    /** Appointment COMPLETED, or medical report finalized (then sync appointment status). */
    private boolean ensureVisitCompleted(Appointment appointment) {
        if (appointment == null) return false;
        if ("COMPLETED".equals(appointment.getStatus())) return true;
        MedicalReport medical = medicalRecordService.findByAppointmentId(appointment.getId());
