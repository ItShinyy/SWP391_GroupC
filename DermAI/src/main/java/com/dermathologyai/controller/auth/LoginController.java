...
public class LoginController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (session != null && session.getAttribute("user") != null) {
            AuthRedirect.afterLogin(req, resp, (User) session.getAttribute("user"));
            return;
        }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (result.status == AuthService.LoginResultStatus.SUCCESS) {
            ...
            auditService.log(user.getId(), "LOGIN_LOCKED", "users", user.getId(), null, "{\"reason\":\"" + reason + "\"}", null, RequestUtil.getClientIp(req), req.getHeader("User-Agent"));
            session.setAttribute("lockedUser", user);
            resp.sendRedirect(req.getContextPath() + "/auth/login");

        } else {
            auditService.log(user != null ? user.getId() : null, "LOGIN_FAILED", "users", null, null, "{\"keyword\":\"" + keyword + "\"}", null, RequestUtil.getClientIp(req), req.getHeader("User-Agent"));
