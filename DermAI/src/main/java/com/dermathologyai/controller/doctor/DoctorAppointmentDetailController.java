        ...
        switch (action) {
            ...
            case "finalizeMedicalReport":
                ...
                success = draft != null && medicalRecordService.finalizeReport(
                    appointmentId, chief, diagnosis, treatmentPlan, prescriptionNote, followUp);
                if (success) {
                    // Medical report done = visit finished → patient can rate
                    if (!"COMPLETED".equals(ownedAppointment.getStatus())) {
                    ...
                    auditService.log(user.getId(), "DOCTOR_FINALIZE_MEDICAL_REPORT", "medical_reports",
                        draft.getId(), null, "{\"status\":\"COMPLETED\"}", null, RequestUtil.getClientIp(req), req.getHeader("User-Agent"));
                }
