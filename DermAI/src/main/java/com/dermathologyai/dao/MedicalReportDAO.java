...
public class MedicalReportDAO extends DBContext {
    ...
    public MedicalReport findByAppointmentId(String appointmentId) {
        return queryOne(
            "SELECT id, appointment_id, doctor_id, diagnosis_report_id, chief_complaint, doctor_diagnosis," +
            " treatment_plan, prescription_note, follow_up_date, status, created_at, updated_at" +
            " FROM medical_reports WHERE appointment_id = ?",
            MedicalReportDAO::mapBasicRow, appointmentId
        );
    }

    public String createDraft(String appointmentId, String doctorId, String diagnosisReportId,
                              String chiefComplaint, String doctorDiagnosis, String treatmentPlan, String prescriptionNote) {
        return insertReturningId(
            "INSERT INTO medical_reports (id, appointment_id, doctor_id, diagnosis_report_id, chief_complaint, doctor_diagnosis," +
            " treatment_plan, prescription_note, status, created_at, updated_at)" +
            " OUTPUT INSERTED.id VALUES (NEWID(), ?, ?, ?, ?, ?, ?, ?, 'DRAFT', SYSDATETIME(), SYSDATETIME())",
            appointmentId, doctorId, diagnosisReportId, chiefComplaint, doctorDiagnosis, treatmentPlan, prescriptionNote
        );
    }

    public boolean finalizeReport(String id, String chiefComplaint, String doctorDiagnosis, String treatmentPlan,
                                  String prescriptionNote, LocalDate followUpDate) {
        return executeUpdate(
            "UPDATE medical_reports SET chief_complaint = ?, doctor_diagnosis = ?, treatment_plan = ?, prescription_note = ?," +
            " follow_up_date = ?, status = 'COMPLETED', updated_at = SYSDATETIME() WHERE id = ? AND status IN ('DRAFT', 'COMPLETED', 'AMENDED')",
            chiefComplaint, doctorDiagnosis, treatmentPlan, prescriptionNote,
            followUpDate == null ? null : Date.valueOf(followUpDate), id
        );
    }

    public int countByStatus(String status) {
        return queryScalar("SELECT COUNT(*) FROM medical_reports WHERE status = ?", status);
    }
    ...
    private static MedicalReport mapBasicRow(ResultSet rs) throws SQLException {
        ...
        report.setDiagnosisReportId(rs.getString("diagnosis_report_id"));
        report.setChiefComplaint(rs.getString("chief_complaint"));
        report.setDoctorDiagnosis(rs.getString("doctor_diagnosis"));
        report.setTreatmentPlan(rs.getString("treatment_plan"));
        report.setPrescriptionNote(rs.getString("prescription_note"));
        Date followUp = rs.getDate("follow_up_date");
        if (followUp != null) report.setFollowUpDate(followUp.toLocalDate());
        report.setStatus(rs.getString("status"));
        Timestamp created = rs.getTimestamp("created_at");
        if (created != null) report.setCreatedAt(created.toLocalDateTime());
        Timestamp updated = rs.getTimestamp("updated_at");
        if (updated != null) report.setUpdatedAt(updated.toLocalDateTime());
        return report;
    }
