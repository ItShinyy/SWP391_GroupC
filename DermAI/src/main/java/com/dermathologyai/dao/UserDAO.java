...
public class UserDAO extends DBContext {
    ...
    private static final String SELECT_COLS =
        "SELECT id, google_id, username, avatar, email, phone, password_hash, full_name, role, status, " +
    ...
    public User findByUsernameOrEmail(String keyword) {
        return queryOne(
            SELECT_COLS + " WHERE email = ? OR username = ?",
            UserDAO::mapRow, keyword, keyword
        );
    }
    ...
    public boolean isPhoneTakenByOther(String phone, String excludeUserId) {
        return queryScalar(
    ...
    public List<User> findAll(String search, String role, String status, String segment, int page, int pageSize) {
        StringBuilder sql = new StringBuilder(SELECT_COLS + " WHERE 1=1");
        List<Object> params = new ArrayList<>();
        appendFilters(sql, params, search, role, status, segment);
    ...
    public int countAll(String search, String role, String status, String segment) {
    ...
    public String create(User user) {
        String sql = "INSERT INTO users (id, google_id, username, avatar, email, phone, password_hash, full_name, role, status, created_at, updated_at)" +
                     " OUTPUT INSERTED.id" +
        ...
        return insertReturningId(sql,
            ...
            user.getFullName(),
            user.getRole()   != null ? user.getRole()   : "PATIENT",
            user.getStatus() != null ? user.getStatus() : "ACTIVE"
        );
    }

    public boolean update(User user) {
        String sql = "UPDATE users SET google_id = ?, username = ?, avatar = ?, email = ?, phone = ?, password_hash = ?," +
                     " full_name = ?, role = ?, status = ?, updated_at = GETDATE() WHERE id = ?";
        return executeUpdate(sql,
            user.getGoogleId(), user.getUsername(), user.getAvatar(), user.getEmail(),
    ...
    private static void appendFilters(StringBuilder sql, List<Object> params,
                                      String search, String role, String status, String segment) {
        ...
        if ("employee".equalsIgnoreCase(segment)) {
            sql.append(" AND role IN ('DOCTOR','ADMIN')");
        } else if ("regular".equalsIgnoreCase(segment)) {
            sql.append(" AND role IN ('PATIENT','USER')");
        }
