...
public class DoctorScheduleDAO extends DBContext {
    ...
    public boolean incrementBookedCount(String scheduleId) {
        return executeUpdate(
            "UPDATE doctor_schedules SET booked_count = booked_count + 1 WHERE id = ? AND is_available = 1 AND booked_count < max_patients",
            scheduleId
        );
    }

    public boolean incrementBookedCountWithConnection(Connection conn, String scheduleId) throws SQLException {
    ...
    public boolean decrementBookedCount(String scheduleId) {
        return executeUpdate(
            "UPDATE doctor_schedules SET booked_count = booked_count - 1 WHERE id = ? AND booked_count > 0",
            scheduleId
        );
    }
