...
<div class="container-fluid pt-3 pb-5">
    ...
    <c:if test="${not empty appointment}">
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a href="${pageContext.request.contextPath}/admin/ai-results/detail?id=${appointment.diagnosisReportId}" class="font-monospace small text-primary bg-primary bg-opacity-10 px-2 py-1 rounded border border-primary text-decoration-none">ID: ${appointment.diagnosisReportId} <i class="fa-solid fa-external-link-alt ms-1"></i></a>
                                </c:otherwise>
                            </c:choose>
                        </div>
                    </div>
        ...
        <div class="card shadow-sm border-0 rounded-4">
            <div class="card-header bg-white border-bottom-0 pt-4 pb-0">
                <h6 class="fw-bold text-dark"><i class="fa-solid fa-receipt text-primary me-2"></i> Thanh toán</h6>
            </div>
            <div class="card-body p-4">
                <c:if test="${param.pay == 'confirmed'}">
                    <div class="alert alert-success">Đã xác nhận thanh toán tiền mặt.</div>
                </c:if>
                <c:if test="${param.pay == 'error'}">
                    <div class="alert alert-danger">Không thể xác nhận thanh toán.</div>
                </c:if>
                <c:choose>
                    <c:when test="${empty invoice}">
                        <p class="text-muted mb-0 small">Chưa có hóa đơn cho lịch hẹn này.</p>
                    </c:when>
                    <c:otherwise>
                        <div class="row g-3 align-items-center">
                            <div class="col-md-4">
                                <div class="text-muted small fw-bold text-uppercase mb-1">Hóa đơn</div>
                                <div><span class="badge text-bg-light border"><c:out value="${invoice.status}"/></span></div>
                                <div class="small text-muted mt-1"><c:out value="${invoice.totalAmount}"/> VND</div>
                            </div>
                            <div class="col-md-4">
                                <div class="text-muted small fw-bold text-uppercase mb-1">Thanh toán gần nhất</div>
                                <c:choose>
