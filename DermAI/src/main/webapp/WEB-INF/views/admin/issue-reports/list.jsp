...
<style>
    .issue-admin-page { min-height: auto; }
    .issue-card { border: 1px solid #e2e8f0; border-radius: 16px; }
    .issue-table thead th { background: #212529; color: #fff; white-space: nowrap; }
    .issue-description { min-width: 140px; max-width: 220px; white-space: normal; font-size: .85rem; }
    .issue-actions { min-width: 245px; }
    .status-badge { font-size: .78rem; padding: .5rem .7rem; }
</style>

<section class="issue-admin-page admin-page admin-page--fit py-2">
    <div class="container-fluid px-3">
        <div class="issue-card bg-white shadow-sm p-3">
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-2 mb-2">
                <div>
                    <h1 class="page-title mb-0"><i class="fa-solid fa-bug text-danger me-2"></i>Issue Reports</h1>
                    <p class="page-subtitle mb-0">Báo cáo sự cố từ người dùng.</p>
                </div>
                            ...
                            <c:otherwise>
                                <c:forEach var="report" items="${reports}">
                                    <tr>
                                        ...
                                        <td class="issue-description small">${report.description}</td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${not empty report.imageUrl}">
                                                    <c:choose>
                                                        <c:when test="${report.imageUrl.startsWith('http')}">
            ...
            <c:set var="pageQuery" value="&amp;status=${selectedStatus}&amp;search=${search}" scope="request"/>
