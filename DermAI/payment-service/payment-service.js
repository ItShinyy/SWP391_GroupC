...
function required(name) {
    ...
    const isPlaceholder = /^(your-|replace-with-|change-me)/i.test(value);
    if (!value || isPlaceholder) {
        throw new PaymentError(500, 'PAYMENT_CONFIGURATION_ERROR', `Missing or placeholder ${name}`);
    }
    return value;
}
...
async function createVnPayPayment(invoiceId, clientIp, locale) {
    ...
    try {
        ...
        const configuredExpireSeconds = Number(process.env.PAYMENT_EXPIRE_SECONDS || 240);
        const expireSeconds = Number.isFinite(configuredExpireSeconds) && configuredExpireSeconds >= 60
            ? configuredExpireSeconds
            : 240;
        const vnpayExpiresAt = new Date(Date.now() + expireSeconds * 1000);
        ...
        const paymentResult = await transaction.request()
            ...
            .input('expireSeconds', sql.Int, expireSeconds)
            .query(`
                ...
                VALUES (
                    ...
                    @orderInfo, @clientIp, DATEADD(SECOND, @expireSeconds, SYSDATETIME())
                );
            `);
        ...
        const paymentUrl = vnpay.buildPaymentUrl({
            ...
            vnp_ExpireDate: dateFormat(vnpayExpiresAt),
        });
