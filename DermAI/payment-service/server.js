...
require('dotenv').config({
    path: process.env.PAYMENT_ENV_FILE || path.join(__dirname, '.env.local'),
});
...
app.get('/payments/vnpay/return', asyncRoute(async (request, response) => {
    ...
    const paymentSucceeded = verification.isSuccess
        && verification.vnp_TransactionStatus === '00';
    const normalizedUiBaseUrl = uiBaseUrl.replace(/\/+$/, '');
    response.redirect(
        302,
        `${normalizedUiBaseUrl}/patient/appointments?pay=${paymentSucceeded ? 'success' : 'failed'}&txnRef=${encodeURIComponent(verification.vnp_TxnRef)}`
    );
}));
